...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getIdentityCards();
    case 'POST':
      return createIdentityCard();
    default:
  ...
  async function getIdentityCards() {
    ...
    const response = await prisma.identity_card
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'identity_card'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createIdentityCard() {
    ...
    const data = await prisma.identity_card.create({
      data: body,
    });
